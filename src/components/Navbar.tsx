...
const navItems: NavItem[] = [
  ...
  { label: "MY MISSION", href: "#advocacy-causes" },
  { label: "GALLERY", href: "#discography" },
  { label: "SUPPORT MY JOURNEY", href: "#hero-support" },
];
        ...
        <nav className="hidden lg:flex items-center gap-8 xl:gap-10">
          {navItems.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className="text-[12px] font-medium tracking-[2px] text-gray-300 hover:text-white transition-colors uppercase py-1 relative group"
            >
              {item.label}
